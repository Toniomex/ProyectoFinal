...
import com.proyectofinal.model.*; // Asegúrate de que tus entidades estén en este paquete
import com.proyectofinal.service.*; // Asegúrate de que tus servicios estén en este paquete
...
public class DemoRunner implements CommandLineRunner {
    ...
    @Autowired
    private UbicacionService ubicacionService; // Asumiendo que existe un UbicacionService con un método guardarUbicacion
     ...
     * Contiene la lógica de demostración de las funcionalidades.
     *
     * @param args Argumentos de línea de comandos (no usados en esta demo).
     * @throws Exception En caso de errores durante la ejecución de la demo.
     */
    @Override
    public void run(String... args) throws Exception {
        ...
        inquilino1.setContraseña("password123"); // ADVERTENCIA: En una aplicación real, las contraseñas deben ser hasheadas (ej. con BCrypt)
        inquilino1.setTelefono("111222333");
        inquilino1.setRol(Persona.Rol.INQUILINO); // Asumiendo que Persona tiene un enum Rol
        try {
            ...
            // Intentar recuperar si ya existe (ej. por mail único)
            Optional<Persona> existingPersona = personaService.autenticarPersona("alice@example.com", "password123");
            if (existingPersona.isPresent()) {
                ...
                return; // Detener la demo si no podemos registrar/recuperar usuarios base
            }
        }
        ...
        inquilino2.setContraseña("securepass");
        ...
        try {
            ...
            Optional<Persona> existingPersona = personaService.autenticarPersona("bob@example.com", "securepass");
            if (existingPersona.isPresent()) {
        ...
        inquilino3.setContraseña("charliepass");
        ...
        try {
            ...
            Optional<Persona> existingPersona = personaService.autenticarPersona("charlie@example.com", "charliepass");
            if (existingPersona.isPresent()) {
        ...
        // 2. Autenticar Persona
        System.out.println("\n--- 2. Autenticando Persona ---");
        Optional<Persona> authenticatedPersona = personaService.autenticarPersona("alice@example.com", "password123");
        if (authenticatedPersona.isPresent()) {
            System.out.println("✔ Autenticación exitosa para: " + authenticatedPersona.get().getNombre());
        } else {
        ...
        try {
            // Asumiendo que UbicacionService tiene un método guardarUbicacion que devuelve la entidad guardada
        ...
        String nifArrendadorA = "A12345678"; // NIF para el primer grupo de inquilinos
        String nifArrendadorB = "B98765432"; // NIF para el segundo grupo de inquilinos

        // Contrato 1 para inquilino1 con arrendador A
        Contrato contrato1 = new Contrato();
        contrato1.setFechaInicio(LocalDate.of(2023, 1, 1));
        contrato1.setFechaFinal(LocalDate.of(2024, 1, 1));
        contrato1.setPrecio(new BigDecimal("1000.00")); // Corregido: Usar BigDecimal
        contrato1.setTipo("Vivienda");
        contrato1.setUbicacion(ubicacion1); // Asociar con la ubicación creada
        contrato1.setNifArrendador(nifArrendadorA); // NIF para la lógica de chat
        contrato1.setInquilino(inquilino1); // Asociar con inquilino1
        try {
            contrato1 = contratoService.crearContrato(contrato1); // Este método debería manejar la lógica de chat
        ...
        // Contrato 2 para inquilino2 con el mismo arrendador A (debería unirse al chat existente)
        Contrato contrato2 = new Contrato();
        contrato2.setFechaInicio(LocalDate.of(2023, 3, 1));
        contrato2.setFechaFinal(LocalDate.of(2024, 3, 1));
        contrato2.setPrecio(new BigDecimal("1100.00")); // Corregido: Usar BigDecimal
        contrato2.setTipo("Vivienda");
        contrato2.setUbicacion(ubicacion1); // Misma ubicación o diferente, no importa para el chat
        contrato2.setNifArrendador(nifArrendadorA); // NIF para la lógica de chat
        contrato2.setInquilino(inquilino2); // Asociar con inquilino2
        try {
        ...
        // Contrato 3 para inquilino3 con un nuevo arrendador B (debería crear un nuevo chat)
        ...
        contrato3.setPrecio(new BigDecimal("950.00")); // Corregido: Usar BigDecimal
        contrato3.setTipo("Local");
        contrato3.setUbicacion(ubicacion2); // Asociar con la segunda ubicación
        contrato3.setNifArrendador(nifArrendadorB); // Nuevo NIF
        contrato3.setInquilino(inquilino3); // Asociar con inquilino3
        try {
        ...
        if (chatA.isPresent()) {
            ...
            mensaje1.setPersona(inquilino1); // Corregido: setPersona
            try {
            ...
            mensaje2.setPersona(inquilino2); // Corregido: setPersona
            try {
        ...
        if (chatB.isPresent()) {
            ...
            mensaje3.setPersona(inquilino3); // Corregido: setPersona
            try {
        ...
        if (chatA.isPresent()) {
            ...
            if (mensajesChatA.isEmpty()) {
                ...
                for (Mensaje msg : mensajesChatA) {
                    // Corregido: Acceso al nombre de la persona a través del objeto Persona en Mensaje
